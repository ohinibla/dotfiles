...
M.get_poetry_envs = function()
	...
	local get_venvs_for = function(base_path, source, opts)
		...
		local paths =
			scan_dir(base_path, vim.tbl_extend("force", { depth = 1, only_dirs = true, silent = true }, opts or {}))
...
M.get_dotfiles_update_git = function()
	vim.loop.spawn("git", {
		...
		if code == 0 then
			...
			handle:close()
			if
				string.match(result, "Your branch is behind")
				or string.match(result, "Your branch and 'origin/main' have diverged")
			then
...
function M.grep_lunarvim_config_folder(opts)
	...
	local theme_opts = themes.get_ivy({
		...
		prompt_prefix = ">> ",
		prompt_title = "~ grep LunarVim config folder ~",
		cwd = get_runtime_dir(),
  ...
  local ignore = {
	...
	opts = vim.tbl_deep_extend("force", theme_opts, ignore, opts)
